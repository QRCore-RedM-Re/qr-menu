...
RegisterNetEvent('qr-menu:client:openMenu', function(data)
    openMenu(data)
end)

RegisterNetEvent('qr-menu:client:closeMenu', function()
    closeMenu()
end)
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isQBCommand then
                    TriggerServerEvent('QRCore:CallCommand', data.params.event, data.params.args)
